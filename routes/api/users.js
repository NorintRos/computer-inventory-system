...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (err.code === 11000) {
        return sendApiError(res, 400, 'Username already exists');
      }
  ...
  async (req, res, next) => {
    try {
      ...
      if (status === 'Disabled' && req.params.id === String(req.user._id)) {
        return sendApiError(res, 400, 'Cannot disable your own account');
      }

      // Load first so we can 404 before mutating anything.
      const existing = await User.findById(req.params.id);
      if (!existing) return sendApiError(res, 404, 'User not found');
